...
{
// BEGIN: T2
    position = operator+(getPosition(), getVelocity());
// END: T2
}
...
// BEGIN: T8
    ...
    // Oppdaterer punktene til trekanten ut i fra kalkulering
    //p1 = {position.x + cos(theta)*size, position.y + sin(theta)*size};
    //p2 = {position.x + cos(theta + 2/3*pi)*size, position.y + sin(theta + 2/3*pi)*size};
    //p3 = {position.x + cos(theta - 2/3*pi)*size, position.y + sin(theta - 2/3*pi)*size};
...
{
    // fjerner alle elementene i friends og foes
    friends.clear();
    foes.clear();

    // iterer gjennom hver element i bird
    for (const auto& bird : birds) {

        // sjekker om det er en due, og om den er innefor friend avstand
        bool isDove = ((bird.get() != this) &&
        (isInstanceOf<Dove>(bird.get())) &&
        (std::sqrt(std::pow(bird->getPosition().x - position.x, 2) + std::pow(bird->getPosition().y - position.y, 2)) < FRIEND_RADIUS));

        // legger den til i friens hvis kriteriene er sanne
        if (isDove) {
            friends.push_back(bird);
        }

        // sjekker om det er en hawk, og om den er innefor avoid avstand
        bool isFoe = ((bird.get() != this) &&
            (isInstanceOf<Hawk>(bird.get())) &&
            (std::sqrt(std::pow(bird->getPosition().x - position.x, 2) + std::pow(bird->getPosition().y - position.y, 2)) < AVOID_RADIUS));
...
{
// BEGIN: T7
    // lagrer først alle bidragene i en variabel
    FloatingPoint newVelocity;
    newVelocity = calculateCohesion() + calculateAlignment() + calculateSeparation() + calculateAvoidance(); 

    if (magnitude(newVelocity) > MAX_SPEED) {
        velocity = (newVelocity*0.9) * MAX_SPEED;
    }
...
{   
    // Oppette en while loop, som kjører koden hvis duen er en del av en flokk
    // Sette displaycolor til fargen samme farge som resten
...
{
// BEGIN: T10
    // opprette et if statement som gjennomfører operasjonen om D-knappen og venste museknapp holdes nede

    bool isDButton = true;
    bool isLeftMouse = false;

    if (isDButton && isLeftMouse) {
        // finner koordinatene til musen, og lagmer den i en variabel
        FloatingPoint position = {100, 100}; // vet ikke hvordan man finner posisijonen til musen ;(
        createDove(position, {5, 5}, Color::red);
    }
